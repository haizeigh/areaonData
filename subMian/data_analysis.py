...
# print(score)
xgbc = XGBC(max_depth=200 , seed=999, n_estimators=100, scale_pos_weight = 8 )
...
effective_columns_data_b = data_b[effective_columns_before]
effective_columns_data_b = effective_columns_data_b.fillna(99)
flag_data_b = pd.DataFrame(data_b['flag'])
# 预检验b的auc
proba_data_b = xgbc.predict_proba(effective_columns_data_b)
print("预检验b的数据，得分auc：")
print(AUC(flag_data_b, proba_data_b[:,1]))

append_data = data_a.append(data_b)
...
xgbc = XGBC(max_depth=200 , seed=999, n_estimators=200, scale_pos_weight = 11 )
